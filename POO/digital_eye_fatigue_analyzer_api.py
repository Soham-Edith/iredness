...
# Try to import tensorflow for local redness model
try:
    ...
    print("Warning: TensorFlow not available. Local redness model will not be used.")
...
class DigitalEyeFatigueAnalyzer:
    def __init__(self, root):
        ...
        # Initialize local redness model (only method for redness detection)
        self.local_redness_model = LocalRednessModel("eye_redness_model.keras")
    ...
    def analyze_eye_fatigue(self):
        ...
        try:
            ...
            # Redness detection - Use local MobileNetV2 model only
            redness_score = None
            
            if self.local_redness_model.model is not None:
                try:
                    redness_score = self.local_redness_model.predict_redness(processed_image_path)
                    if redness_score is not None:
                        results['redness'] = redness_score
                        api_success = True
                        print(f"Local redness model prediction: {redness_score:.3f}")
                    else:
                        results['redness'] = None
                        print("Warning: Local redness model returned None")
                except Exception as e:
                    print(f"Local redness model error: {e}")
                    results['redness'] = None
            else:
                print("Error: Local redness model not available. Please ensure the model file exists.")
                results['redness'] = None
...
class LocalRednessModel:
    """Helper class for local MobileNetV2 redness detection model"""
    
    def __init__(self, model_path="eye_redness_model.keras"):
        self.model = None
    ...
    def load_model(self):
        """Load the trained MobileNetV2 model (trained on TensorFlow 2.19)"""
        if not TENSORFLOW_AVAILABLE:
            print("TensorFlow not available. Cannot load local redness model.")
            print("Please install TensorFlow 2.19.0: pip install tensorflow==2.19.0")
            return
        ...
        try:
            ...
            if not tf_version.startswith('2.'):
                print(f"Warning: Model was trained on TensorFlow 2.19, current version is {tf_version}")
            
            if os.path.exists(self.model_path):
                self.model = tf.keras.models.load_model(self.model_path)
                print(f"Local redness model loaded successfully from {self.model_path}")
            else:
                print(f"Error: Model file not found at {self.model_path}")
                print(f"Please ensure the model file 'eye_redness_model.keras' is in the project directory.")
        except Exception as e:
            print(f"Error loading local redness model: {e}")
            print("Please ensure TensorFlow 2.19.0 is installed and the model file exists.")
    
    def predict_redness(self, image_path):
        """
        Predict redness score from image using local model.
        
        Args:
            image_path: Path to the image file
            
        Returns:
            float: Redness score in range [0, 1] (0 = normal, 1 = red)
        ...
        try:
            ...
            img = tf_image.load_img(image_path, target_size=self.img_size)
            ...
            # Convert from [0, 1] probability to [0, 1] redness score
            # The model outputs probability of "red eye", so we use it directly
            redness_score = float(prob)
            
            return redness_score
        except Exception as e:
            print(f"Error predicting redness with local model: {e}")
            return None


class DigitalEyeFatigueAnalyzerEye:
    """API-friendly class for eye fatigue analysis without GUI dependencies"""
    
    def __init__(self, redness_model_path="eye_redness_model.keras"):
        # Initialize Roboflow client
        self.api_key = "hBljhOwZ15CdLgOj0Kjj"
        self.client = InferenceHTTPClient(
            api_url="https://serverless.roboflow.com",
        ...
        # Initialize local redness model (only method for redness detection)
        self.local_redness_model = LocalRednessModel(redness_model_path)
    ...
    def normalize_and_calibrate(self, raw):
        """Normalize and calibrate raw score"""
        if raw is None:
            return 0.0
        try:
            if raw < 0.5:
    ...
    def analyze_image(self, image_path, screen_time_hours=8.0, condition="Normal"):
        """
        Analyze eye fatigue from an image file.
        
        Args:
            image_path: Path to the image file
            screen_time_hours: Hours of screen time (default: 8.0)
            condition: Eye condition (default: "Normal")
        
        Returns:
            dict: Analysis results with 'success', 'redness', 'dryness', 'fatigue' keys
        """
        try:
            # Load image
            image = Image.open(image_path)
            ...
            # Preprocess image
            bgr_processed, processed_base64 = self.preprocess_image_pil(image)
            ...
            # Fatigue detection
            try:
                fatigue_result = self.client.infer(processed_temp_path.name, model_id=self.fatigue_model)
                fatigue_score = self.extract_confidence(fatigue_result, "fatigue")
                results['fatigue'] = fatigue_score
            except Exception as e:
                print(f"Fatigue model error: {e}")
            ...
            # Dryness detection
            try:
                dryness_result = self.client.infer(processed_temp_path.name, model_id=self.dryness_model)
                dryness_score = self.extract_confidence(dryness_result, "dryness")
                results['dryness'] = dryness_score
            except Exception as e:
                print(f"Dryness model error: {e}")
            ...
            if self.local_redness_model.model is not None:
                try:
                    redness_score = self.local_redness_model.predict_redness(processed_temp_path.name)
                    if redness_score is not None:
                        results['redness'] = redness_score
                        print(f"Local redness model prediction: {redness_score:.3f}")
                    else:
            ...
            # Use demo values if API calls failed
            f_raw = results.get('fatigue', 0.65) if results.get('fatigue') is not None else 0.65
            r_raw = results.get('redness', 0.35) if results.get('redness') is not None else 0.35
            d_raw = results.get('dryness', 0.45) if results.get('dryness') is not None else 0.45
            
            # Normalize scores
            f = self.normalize_and_calibrate(f_raw)
            r = self.normalize_redness_score(r_raw)
            d = self.normalize_and_calibrate(d_raw)
            ...
            return {
                'success': True,
                'redness': round(r_10, 1),
                'dryness': round(d_10, 1),
                'fatigue': round(final_10, 1)
            }
